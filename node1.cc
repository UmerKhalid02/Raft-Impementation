...
struct Node{
  ...
  int id = 1;
  int status; // 0 = follower, 1 = candidate, 2 = leader , 2 = leader
  int term_num;
  bool hasVoted = false;
...
void RunServer() {
  ...
  std::cout << "Node 1 listening on " << server_address << std::endl;
...
int main(int argc, char** argv) {
  ...
  // config file
  std::vector<std::pair<int, std::string>> Votes = {{2, "no"}, {3, "no"}, {4, "no"},{5, "no"}}; // { "id", "vote" }
  std::vector<std::string> addresses = {"localhost:50052", "localhost:50053", "localhost:50054", "localhost:50055"}; // for config file
