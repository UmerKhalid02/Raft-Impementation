...
void RunServer() {
  ...
  std::cout << "Node 2 listening on " << server_address << std::endl;
...
int main(int argc, char** argv) {
  ...
  std::thread server_thread([] {RunServer();});
    ...
    // config file
  std::vector<std::pair<int, std::string>> Votes = {{1, "no"}, {3, "no"}, {4, "no"},{5, "no"}}; // { "id", "vote" }
  std::vector<std::string> addresses = {"localhost:50051", "localhost:50053", "localhost:50054", "localhost:50055"};
