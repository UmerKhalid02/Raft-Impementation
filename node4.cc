...
struct Node{
  std::string address = "localhost:50054";
  int id = 4;
  int status;  // 0 = follower, 1 = candidate , 2 = leader
  int term_num;
  bool hasVoted = false;
  std::string logfile = "log4.txt";
...
void RunServer() {
  ...
  std::cout << "Node 4 listening on " << server_address << std::endl;
...
int main(int argc, char** argv) {
  ...
  std::thread server_thread([] {RunServer();});
    ...
    // config file
  std::vector<std::pair<int, std::string>> Votes = {{1, "no"}, {2, "no"}, {3, "no"},{5, "no"}}; // { "id", "vote" }
  std::vector<std::string> addresses = {"localhost:50051", "localhost:50052", "localhost:50053", "localhost:50055"};
